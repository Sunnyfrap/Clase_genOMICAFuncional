##WGCNA, its edited to choose second best
...
##############################################################
# 46B. SELECCIÓN DEL SEGUNDO MEJOR MÓDULO OBJETIVO
##############################################################

# Selecciona dinámicamente la fila 2 de los resultados ordenados por p-value
best.module.name <- interaction.results$module[2] 
module.genes <- names(moduleColors[moduleColors == best.module.name])

cat(
  "\n============================================\n",
  "SEGUNDO MEJOR MÓDULO SELECCIONADO:", best.module.name,
...
write.csv(
  hub.table,
  file.path(outpath, paste0("WGCNA_hub_genes_", best.module.name, ".csv")),
  row.names = FALSE
)
...
plot(
  ...
  main = paste("Módulo (2° mejor):", best.module.name, "- Interacción CR95 vs Catuai")
)
legend(
  "topleft",
...
##############################################################
# 53. CONVERSIÓN A CSV Y CONEXIÓN CON CYTOSCAPE
##############################################################

while (dev.cur() > 1) dev.off()
graphics.off()

# Conversión automática de TXT a CSV dinámico
edges_txt <- file.path(outpath, paste0("Cytoscape_edges_", best.module.name, ".txt"))
edges_df  <- read.csv(edges_txt, sep = "", header = TRUE, check.names = FALSE)
write.csv(
  edges_df, 
  file.path(outpath, paste0("Cytoscape_edges_", best.module.name, ".csv")), 
  row.names = FALSE
)

nodes_txt <- file.path(outpath, paste0("Cytoscape_nodes_", best.module.name, ".txt"))
if (file.exists(nodes_txt)) {
  nodes_df <- read.csv(nodes_txt, sep = "", header = TRUE, check.names = FALSE)
  write.csv(
    nodes_df, 
    file.path(outpath, paste0("Cytoscape_nodes_", best.module.name, ".csv")), 
    row.names = FALSE
  )
}
...
tryCatch({
  ...
  edges_df <- read.csv(file.path(outpath, paste0("Cytoscape_edges_", best.module.name, ".csv")))
  nodes_df <- read.csv(file.path(outpath, paste0("Cytoscape_nodes_", best.module.name, ".csv")))
  ...
  style_name <- paste0("WGCNA_Style_", best.module.name)
  ...
  cat("\n[AVISO] No se pudo conectar a Cytoscape (Asegúrate de que la aplicación esté abierta):\n", e$message, "\n")
})

cat("\n============================================\n")
cat("ANÁLISIS COMPLETO Y ARCHIVOS GENERADOS\n")
cat("Módulo procesado:", best.module.name, "\n")
cat("Ruta de salida:", outpath, "\n")
cat("============================================\n")